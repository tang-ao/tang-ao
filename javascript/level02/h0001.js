...
nav02.addEventListener('click', function() {
  ...
  if (dir) {
    nowpage.setAttribute('class', 'move-out-right'); //当前页出去
    page02.setAttribute('class', 'move-in-right'); //页面一进来
  } else {
    // 切换页面动画 当前的页面套用出去的动画，页面二套用进来的动画
    nowpage.setAttribute('class', 'move-out'); //当前页出去
    page02.setAttribute('class', 'move-in'); //页面一进来
  }
  // 页面二成为当前页面
  nowpage = page02;
});
...
nav03.addEventListener('click', function() {
  ...
  if (dir) {
    nowpage.setAttribute('class', 'move-out-right'); //当前页出去
    page03.setAttribute('class', 'move-in-right'); //页面一进来
  } else {
    nowpage.setAttribute('class', 'move-out'); //当前页出去
    page03.setAttribute('class', 'move-in'); //页面一进来
  }
  nowpage = page03;
});
